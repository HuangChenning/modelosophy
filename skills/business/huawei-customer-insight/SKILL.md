...
2. **搜索补充与置信度标注**：优先通过 WebSearch 补充客户官网、财报、新闻资讯，并对每条关键信息标注置信度：**【已证实】**（有明确来源，可追溯到具体页面/公告/文章）/**【推断】**（基于事实的合理推理，需说明依据）/**【待核实】**（缺乏直接证据，需在接触中验证）。无法确认的信息一律标【待核实】，严禁捏造。
3. **分析深度自适应判定**：
...
- 产出”客户生意说明书”，标注每条信息的置信度（【已证实】/【推断】/【待核实】）与来源。
...
- **误用 1：与通用企业情报报告混淆**
  - *纠正*：通用企业情报报告（如 `org-it-intel-report`）侧重于泛化的组织架构与 IT 招投标公开信息搜集，产出的是不带销售判断的客观情报；本模型侧重于 **华为 MCR 销售攻坚体系**，必须包含 UB/SP/TB/EB 决策链权力地图、竞争格局与 $APPEALS 八维度竞争对标。两者是各自独立、可单独使用的技能——本模型不要求、也不依赖 `org-it-intel-report` 先跑一遍，Step 2/3 的业务全景与决策链调研在本模型内自成一套、独立完整。
...
- **误用 3：将信息缺失处自行捏造填补**
  - *纠正*：无法通过搜索或用户输入确认的信息（如内部暗流、未公开政治关系），必须按【待核实】标注并说明需要在下次接触中验证，严禁虚构。
...
- **与[org-it-intel-report](../org-it-intel-report/SKILL.md)**：两者是并列、各自独立可单独使用的技能，互不依赖。`org-it-intel-report` 产出不带销售判断的**客观情报报告**（组织架构、IT招投标、决策链客观事实），适合售前调研/汇报准备场景；本模型产出**MCR 销售攻坚分析**（决策链攻坚判断、竞争格局、$APPEALS、客户分级），适合需要制定打法的场景。两者的业务全景/决策链调研步骤各自独立完整，不要求先跑另一个。
