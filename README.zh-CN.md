...
- [`skills/ThinkingModels/`](skills/ThinkingModels)：思维模型库目前 31 个 Skill（原书 30 个 + 苏格拉底式质疑），覆盖决策代价、认知偏差、系统与战略工具（SWOT、MECE、PDCA、飞轮等），以及前景理论 / 福格 / 黄金圈 / 周哈里窗 / 邓宁-克鲁格等。每个都按真实学科来源重新研究（而非照抄《万物皆模型》原书卡片），经过苏格拉底式质疑自检、盲测用例验证、以及官方 Skill 规范校验。完整区分地图见 [`skills/ThinkingModels/README.md`](skills/ThinkingModels/README.md)。
...
思维模型库仍处于早期，目前覆盖 31 个模型（原书 30 个 + 方法论补充苏格拉底式质疑）。100 个模型里大多数还没开始蒸馏。随着库的扩张，Skill 的编写规范仍可能调整。
