...
4. **先给每条 KR 标类型，再设挑战度。** 二分（Google 实践）：**承诺型 committed** —— 目标即 **1.0**，做不到算事故，须走升级与复盘（合规交付、SLA、安全整改、客户承诺一律属此类）；**挑战型 aspirational** —— 才用 **0.7** 校准，全部稳稳 1.0 说明 KR 太保守。判据：未标类型的 KR 不得评分；把承诺型打 0.7 当"健康挑战"是误用。
