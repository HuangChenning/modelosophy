...
**这 350 个的完成度并不一样，而且这个差别很重要。** 其中只有 **70** 个走完了完整流程——从一手学科来源蒸馏、过了强制的苏格拉底式质疑自检、并做过盲测——标记为 `version: v1.0`。另外 **280** 个是 `v0.x-draft`：按主题名录批量撰写，通过了结构校验、配了最小 evals，但**从未盲测**，内容正确性审查也是最近才开始做。草稿可以当有用的起点，但不要当作已核验的参考资料。每个 Skill 的 `metadata.version` 字段标明了它属于哪一类。
...
- **evals 深度不均。** `thinking-models` 平均每个 Skill 约 10 条测试用例且做过盲测；其余所有分类平均只有 5–6 条，且从未盲测。有 evals 文件不等于该 Skill 的触发判断已被验证。
- **内容审查只做了一部分。** 2026-08-19 的一轮审计精读了六个批量生成分类中的 123 个文件，修掉 3 处实质问题（公式下标被形近的非拉丁字符顶替、一处经典实验细节记错、两个同名但不同的公式撞名未做区分）。质量比预期好，但覆盖并不完整——具体哪些分类和文件审过、哪些没审，见 [`ROADMAP.md`](ROADMAP.md)。
