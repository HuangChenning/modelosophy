...
**这 372 个的完成度并不一样，而且这个差别很重要——但 `v1.0` 这个标签要按窄口径理解。** 92 个标记为 `version: v1.0`：从一手学科来源蒸馏、过了强制的苏格拉底式质疑自检、并由独立 agent 做过盲测。另外 280 个是 `v0.x-draft`：大部分按主题名录批量撰写，只过了结构校验、配了最小 evals，从未盲测；其中一小部分（2026-08-20 已完成独立盲测与 QA 校验，升至 `v1.0`）。**"盲测"验证的是触发判断——skill 会不会在对的场景下被正确调用——不是内容本身的事实准确性。** 2026-08-19 的一轮内容审计对 `thinking-models` 这批 v1.0 也做了审查，同样找出了实质问题（过时的路由表、引用张冠李戴、例证违反正文自己定的规则），出错率并不明显低于草稿。把 `v1.0` 理解为"过了触发测试"，不是"已核事实"。具体哪些文件真正做过内容审查，见 [`ROADMAP.md`](ROADMAP.md)。
...
| [学习与成长](skills/learning-growth/README.md) | 30 | 12 | 18 | [目录](#学习与成长30) |
| [战略与竞争](skills/strategy-competition/README.md) | 30 | 10 | 20 | [目录](#战略与竞争30) |
| [效率与执行](skills/efficiency-execution/README.md) | 30 | 11 | 19 | [目录](#效率与执行30) |
