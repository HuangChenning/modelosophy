...
- **与[KPI](../kpi-framework/SKILL.md)**：用 KPI 框架设计少而尖的关键绩效指标：对齐目标、可测、有主人、分清领先/滞后…。本模型与其形成场景或方法层面的互补/分流。
- **与[平衡计分卡](../balanced-scorecard/SKILL.md)**：用平衡计分卡把战略翻译成财务、顾客、内部流程、学习与成长四类目标，并为每个目标配…。本模型与其形成场景或方法层面的互补/分流。
