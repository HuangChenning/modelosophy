...
| [博弈论与策略](skills/game-theory-models/README.md) | 31 | 31 | 0 | [目录](#博弈论与策略31) |
...
- **全库已 100% 升阶至 `v1.0`。** 全库 373 个 Skill 均已完成盲测用例拓展（每项 $\ge 10$ 条测试用例，涵盖应触发/不应触发/边界/2条以上正确性陷阱）、苏格拉底自检与结构校验。
- **所有 373 个 Skill 的评估集均已配齐 $\ge 10$ 条用例。** 覆盖应触发、不应触发、边界情况及 2+ 条正确性陷阱，全库 3,737 条用例经过 Batch Benchmark 自动化评测（综合通过率 99.65%）。须注意：evals 验证的是触发与分流准确性，而非事实核查。
...
- **[base-rate](skills/decision-probability/base-rate/SKILL.md)** — 基率/参考类：先锚定同类频率再并入个案。
- **[bayesian-updating](skills/decision-probability/bayesian-updating/SKILL.md)** — 贝叶斯更新：先验 × 似然 → 后验。
- **[decision-journal](skills/decision-probability/decision-journal/SKILL.md)** — 决策日志：预注册信念与预测，事后校准。
- **[decision-tree](skills/decision-probability/decision-tree/SKILL.md)** — 决策树：在选项已知、结果不确定、能粗估概率与后果时，做多阶段选择与期望值比较。
- **[expected-utility](skills/decision-probability/expected-utility/SKILL.md)** — 期望效用：金额非线性或存在归零约束时用 EU。
- **[expected-value](skills/decision-probability/expected-value/SKILL.md)** — 期望值：把不确定结果量化为概率加权平均（EV = Σ pᵢ·xᵢ）。
- **[grey-thinking](skills/decision-probability/grey-thinking/SKILL.md)** — 灰度认知：用可修订置信度替代非黑即白。
- **[loss-function](skills/decision-probability/loss-function/SKILL.md)** — 损失函数：非对称错误代价下的行动/阈值。
- **[monte-carlo](skills/decision-probability/monte-carlo/SKILL.md)** — 蒙特卡洛：对不确定输入抽样得结果分布。
- **[mvp](skills/decision-probability/mvp/SKILL.md)** — MVP：以最小投入验证最危险假设。
- **[ooda-loop](skills/decision-probability/ooda-loop/SKILL.md)** — OODA：观察—定向—决策—行动的对抗节奏。
- **[oz-principle](skills/decision-probability/oz-principle/SKILL.md)** — 奥兹原则：Above the Line 问责四步。
- **[planning-fallacy](skills/decision-probability/planning-fallacy/SKILL.md)** — 计划谬误：用外部视图纠偏乐观工期/成本。
- **[pre-mortem](skills/decision-probability/pre-mortem/SKILL.md)** — 事前验尸：假定已失败以挖出可预防原因。
- **[probability-thinking](skills/decision-probability/probability-thinking/SKILL.md)** — 概率论思维：事件定义与概率语言入口。
- **[rapid-experimentation](skills/decision-probability/rapid-experimentation/SKILL.md)** — 快速试错：有止损的短周期实验序列。
- **[red-team](skills/decision-probability/red-team/SKILL.md)** — 红队：独立结构化对抗以找出可利用裂口。
- **[reversible-irreversible](skills/decision-probability/reversible-irreversible/SKILL.md)** — 可逆/不可逆决策：按反悔成本匹配速度与审查。
- **[risk-premium](skills/decision-probability/risk-premium/SKILL.md)** — 风险溢价：无风险基准之上的风险补偿口径。
- **[scenario-planning](skills/decision-probability/scenario-planning/SKILL.md)** — 情景规划：少数分歧未来下的稳健选项与预警。
- **[sensitivity-analysis](skills/decision-probability/sensitivity-analysis/SKILL.md)** — 敏感性分析：找翻转假设与稳健区。
- **[threshold-effect](skills/decision-probability/threshold-effect/SKILL.md)** — 阈值效应：剂量/规则门槛（≠临界点相变）。
...
- **[cognitive-load](skills/learning-growth/cognitive-load/SKILL.md)** — 认知负荷理论：工作记忆容量有限，须把内在/外在/相关负荷控制在容量内。
...
- **[dual-coding](skills/learning-growth/dual-coding/SKILL.md)** — 双重编码：言语与示意表征对齐互译，增加提取线索。
- **[elaborative-interrogation](skills/learning-growth/elaborative-interrogation/SKILL.md)** — 精细加工提问：对命题追问「为什么合理」以锚进知识网。
...
- **[habit-formation](skills/learning-growth/habit-formation/SKILL.md)** — 习惯养成：靠稳定情境线索把行为自动化，纠偏「21天养成」流言。
- **[interleaved-practice](skills/learning-growth/interleaved-practice/SKILL.md)** — 交错练习：混合易混题型以提升辨别与迁移。
- **[learning-by-teaching](skills/learning-growth/learning-by-teaching/SKILL.md)** — 学习中教：用教学任务强制提取与组织知识。
- **[learning-pyramid](skills/learning-growth/learning-pyramid/SKILL.md)** — 学习金字塔（谨慎）：纠偏假百分比，降级为主动参与启发式。
- **[mentorship](skills/learning-growth/mentorship/SKILL.md)** — 导师制：目标—节奏—反馈—渐撤的带教协议。
- **[meta-learning](skills/learning-growth/meta-learning/SKILL.md)** — 元学习：跨任务选择并实验「如何学」的策略菜单。
- **[metacognition](skills/learning-growth/metacognition/SKILL.md)** — 元认知：监控并调节自己的认知过程——觉察是否理解、是否用错策略并换方法。
- **[peer-learning](skills/learning-growth/peer-learning/SKILL.md)** — 同伴学习：独立作答→找持不同意见者讨论→独立重答，用认知冲突逼出理解修正。
- **[production-effect](skills/learning-growth/production-effect/SKILL.md)** — 产出效应：同一份材料里被读出/写出的条目比默读的更好记，但依赖有对照的默读条目。
- **[project-based-learning](skills/learning-growth/project-based-learning/SKILL.md)** — 项目式学习：围绕真实问题长周期自主推进、边解决问题边习得知识技能。
- **[retrieval-practice](skills/learning-growth/retrieval-practice/SKILL.md)** — 检索练习：合上材料主动提取（测试效应），强化保持与诊断。
- **[self-explanation](skills/learning-growth/self-explanation/SKILL.md)** — 自我解释：研读材料时主动补上「为什么这样成立」的推理句，暴露并修复理解缺口。
- **[sleep-consolidation](skills/learning-growth/sleep-consolidation/SKILL.md)** — 睡眠与记忆巩固：新编码记忆在睡眠中被主动巩固整合，清醒状态下效率明显更低。
- **[spaced-repetition](skills/learning-growth/spaced-repetition/SKILL.md)** — 间隔重复：主动检索 + 递增间隔的 SRS/复习日程。
- **[t-shaped-skills](skills/learning-growth/t-shaped-skills/SKILL.md)** — T 型技能：一纵深可交付 + 多横杠可对话的能力组合。
- **[ten-thousand-hours](skills/learning-growth/ten-thousand-hours/SKILL.md)** — 一万小时定律（澄清）：纠偏时数神话，导流刻意练习。
- **[transfer-of-learning](skills/learning-growth/transfer-of-learning/SKILL.md)** — 迁移学习：显式设计近/远迁移变式与跨情境验收。
...
- **[ansoff-matrix](skills/strategy-competition/ansoff-matrix/SKILL.md)** — 安索夫矩阵：现有/新市场×产品四象限增长路径与风险阶梯。
- **[balanced-scorecard](skills/strategy-competition/balanced-scorecard/SKILL.md)** — 平衡计分卡：把已有战略目标配上指标/目标值/行动方案，形成可持续运行的度量系统。
...
- **[business-model-canvas](skills/strategy-competition/business-model-canvas/SKILL.md)** — 商业模式画布：一页九模块描述组织如何创造、传递、获取价值（描述性，不评判好坏）。
- **[core-competence](skills/strategy-competition/core-competence/SKILL.md)** — 核心能力：跨业务共享、难模仿、打开多市场的能力组合。
- **[cost-leadership](skills/strategy-competition/cost-leadership/SKILL.md)** — 成本领先：广阔市场结构性最低成本优势。
- **[differentiation-strategy](skills/strategy-competition/differentiation-strategy/SKILL.md)** — 差异化战略：全市场可感知独特性与溢价。
- **[disruptive-innovation](skills/strategy-competition/disruptive-innovation/SKILL.md)** — 颠覆式创新：低端/新市场轨迹与在位者非对称激励。
- **[dynamic-capabilities](skills/strategy-competition/dynamic-capabilities/SKILL.md)** — 动态能力：环境快变时持续感知-抓住-重构资源的能力，是对静态 RBV/VRIO 的直接批评。
...
- **[kpi-framework](skills/strategy-competition/kpi-framework/SKILL.md)** — KPI 框架：少而尖关键指标与 Goodhart 警示。
- **[mckinsey-7s](skills/strategy-competition/mckinsey-7s/SKILL.md)** — 麦肯锡 7S：硬三/软三绕共同价值观对齐，诊断战略落地的组织对齐。
- **[niche-strategy](skills/strategy-competition/niche-strategy/SKILL.md)** — 利基/集中战略：窄战场上的成本或差异化焦点。
- **[okr](skills/strategy-competition/okr/SKILL.md)** — OKR：鼓舞性目标 + 可度量关键结果对齐与复盘（非待办清单）。
- **[platform-strategy](skills/strategy-competition/platform-strategy/SKILL.md)** — 平台战略：多边市场、网络效应、冷启动与治理变现。
- **[porters-diamond](skills/strategy-competition/porters-diamond/SKILL.md)** — 波特钻石模型：国家/地区产业集群为什么能持续产出全球领先企业（≠波特五力）。
- **[porters-five-forces](skills/strategy-competition/porters-five-forces/SKILL.md)** — 波特五力：在行业层面诊断利润结构——竞争、进入、替代、买方与供方议价。
- **[positioning](skills/strategy-competition/positioning/SKILL.md)** — 定位：目标顾客心智中的清晰可防守位置。
- **[resource-based-view](skills/strategy-competition/resource-based-view/SKILL.md)** — 资源基础观：内生异质资源/能力解释可持续优势。
- **[second-curve](skills/strategy-competition/second-curve/SKILL.md)** — 第二曲线：在第一增长曲线趋缓前启动下一条 S 曲线并管理输血/停投。
- **[strategic-clock](skills/strategy-competition/strategic-clock/SKILL.md)** — 战略钟：价格×感知价值竞争路径，识别夹在中间。
- **[strategic-intent](skills/strategy-competition/strategic-intent/SKILL.md)** — 战略意图：资源弱势的挑战者主动设定十年级雄心，把落差当动员杠杆而非削减雄心。
- **[strategy-map](skills/strategy-competition/strategy-map/SKILL.md)** — 战略地图：只画四层面目标因果链条本身（不含指标），BSC 第 2 步的深潜工具。
...
- **[buffer-time](skills/efficiency-execution/buffer-time/SKILL.md)** — 缓冲时间管理：拆下各任务安全余量集中到少数关键位置，用消耗速度而非单任务是否按期预警（关键链≠关键路径）。
- **[checklist-sop](skills/efficiency-execution/checklist-sop/SKILL.md)** — 清单/SOP：关键步骤外部化，降低遗漏与方差。
- **[commitment-devices](skills/efficiency-execution/commitment-devices/SKILL.md)** — 承诺机制：在冷静状态下给未来行为绑上外部强制代价，让半途而废比做完更差。
- **[continuous-improvement](skills/efficiency-execution/continuous-improvement/SKILL.md)** — 持续改进：小步全员常态化抬高标准。
- **[critical-path](skills/efficiency-execution/critical-path/SKILL.md)** — 关键路径：项目依赖网络中决定最短工期的最长链。
- **[delegation](skills/efficiency-execution/delegation/SKILL.md)** — 委派：结果与权限清晰移交并保留跟进。
- **[eat-the-frog](skills/efficiency-execution/eat-the-frog/SKILL.md)** — 吃青蛙：当日最高杠杆难事先做完。
- **[eisenhower-matrix](skills/efficiency-execution/eisenhower-matrix/SKILL.md)** — 艾森豪威尔矩阵：按紧急×重要分配注意力，优先投入重要但不紧急的事项。
- **[energy-management](skills/efficiency-execution/energy-management/SKILL.md)** — 精力管理：按身心高峰匹配任务类型。
- **[five-s](skills/efficiency-execution/five-s/SKILL.md)** — 5S：整理整顿清扫清洁素养，稳固现场/数字工作区基础。
- **[gantt-chart](skills/efficiency-execution/gantt-chart/SKILL.md)** — 甘特图：时间条与依赖可视化沟通进度。
- **[gtd](skills/efficiency-execution/gtd/SKILL.md)** — GTD：收集-澄清-组织-回顾，落到可信的下一步行动。
- **[inbox-zero](skills/efficiency-execution/inbox-zero/SKILL.md)** — 收件箱清零法：核心不是未读数字归零，而是把「还没处理」的心智负担清空。
...
- **[meeting-hygiene](skills/efficiency-execution/meeting-hygiene/SKILL.md)** — 会议卫生：议程先行、唯一决策责任人、时间盒、控制人数等已验证有效的会议设计规则集合。
- **[milestone-planning](skills/efficiency-execution/milestone-planning/SKILL.md)** — 里程碑规划：可验证阶段门对齐范围与日期。
...
- **[task-switching-cost](skills/efficiency-execution/task-switching-cost/SKILL.md)** — 任务切换成本：切换任务后反应变慢、错误率升高，即使有准备时间也无法完全消除（single-tasking 的诊断层）。
- **[time-blocking](skills/efficiency-execution/time-blocking/SKILL.md)** — 时间块：日历切成专属用途连续时段。
- **[two-minute-rule](skills/efficiency-execution/two-minute-rule/SKILL.md)** — 两分钟法则：澄清时立刻完成≤2 分钟行动。
- **[weekly-review](skills/efficiency-execution/weekly-review/SKILL.md)** — 每周回顾：固定时间盒清空-核对-重定向三件事，可独立于 GTD 使用。
